...
**This is the `poetry` branch.** Sibling branches demonstrate the same project with other dependency
managers: `main` (uv) and `pdm`. CI builds all three. A change to the demo story often needs porting
across them, which means translating the dependency-manager specifics rather than cherry-picking
verbatim.
...
| Command | Effect |
|---|---|
| `./pw build` | install + lint + test + `poetry build` (what CI runs, as `python pw --clean build`) |
| `./pw check` | lint + test |
| `./pw test` | `poetry run pytest` — append args, e.g. `./pw test tests/test_cowsay.py::test_cowsay` |
| `./pw lint` / `./pw format` | ruff check / ruff format + import sort |
| `./pw install` | `poetry install` |
...
- `[tool.pyprojectx.<name>]` blocks each define an isolated tool context. `main` holds poetry/ruff/
...
- `pw.lock` pins the exact tool versions per context (separate from `poetry.lock`, which pins
  project dependencies). Regenerate it with `./pw --lock`, not by hand.
- Project metadata is duplicated: `[project]` carries the PEP 621 fields pyprojectx reads, while
  `[tool.poetry*]` holds the dependencies and the `poetry-core` build backend. `poetry.toml` pins
  `virtualenvs.in-project` so the venv lands in `.venv`.

...
no uv, no ruff, not even pyprojectx itself.
...
| `./pw build` | sync dependencies, lint, test and build a wheel |
| `./pw uv --help` or `./pw https --help` | run uv or HTTPie without ever installing them |
...
requirements = ["uv", "ruff", "prek", "px-utils", "httpie"]
...
* **Contexts are isolated.** `[tool.pyprojectx.jupyter]` keeps JupyterLab's dependencies away from the
  `main` context, and `[tool.pyprojectx.venv]` is the project's own uv-managed `.venv`.
...
This branch uses [uv](https://docs.astral.sh/uv/), but pyprojectx doesn't care which dependency manager
you prefer: the same demo is available with [PDM](https://github.com/houbie/px-demo/tree/pdm) and
